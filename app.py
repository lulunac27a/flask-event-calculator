...
class Event(db.Model):
    ...
    name: int = db.Column(db.String(80), nullable=False)  # event name
    ...
    user_id: int = db.Column(db.Integer, db.ForeignKey(User.id))  # user id
...
def next_event_date_filter(
    ...
    repeat_often: int,
    times_completed: int,
) -> date:  # calculate next recurring event date
    ...
    else:  # if event does not repeat
        return original_due_date  # return original due date
...
def index() -> str:  # get index page template
    events: list = Event.query.order_by(
        Event.due_date
    ).all()  # get list of all events sorted by due date
    user: Union[User, None] = User.query.first()  # get first user in database
...
def add_event() -> Response:  # add event to database
    name: str = request.form.get("name")  # get name from add event form
    original_due_date: str = request.form.get(
        "start_date")  # get original due date
...
def complete_event(event_id) -> Response:  # complete event from event id
    event: Union[Event, None] = Event.query.get(
        event_id)  # find event by event id
...
def delete_event(event_id) -> Response:  # delete event from event id
    event: Union[Event, None] = Event.query.get(
        event_id)  # find event by event id
    if event:
